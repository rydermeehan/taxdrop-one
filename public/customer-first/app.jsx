/* TaxDrop One — Customer First design artifact (progressive intake, 2026-07-04).
   One step at a time: address → optional CAD upload → analyze. Upcoming steps
   stay collapsed so the homeowner focuses on a single action. Mounted at
   /customer-first for review alongside the live /v2 intake.

   Engine + results are identical to /v2/app.jsx; only the intake presentation
   differs. */
  ...
  return (
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: narrow ? "13px 16px" : "16px 40px", background: "#ffffff", borderBottom: "1px solid #e2e8e2" }}>
      <div style={{ display: "flex", alignItems: "center", gap: narrow ? 10 : 14, minWidth: 0 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".08em", color: "#2c8350", background: "#e3efe6", border: "1px solid #cfe6d8", borderRadius: 20, padding: "4px 10px" }}>PROGRESSIVE STEPS</span>
      </div>
...
function StepNum({ n, muted }) {
  return (
    <div style={{
      ...
      background: muted ? "#eef2f0" : "#1d6b41",
      color: muted ? "#9aa6a0" : "#fff",
      display: "flex", alignItems: "center", justifyContent: "center",
      fontSize: 13, fontWeight: 700, flexShrink: 0,
      border: muted ? "1.5px solid #dde5df" : "none",
    }}>{n}</div>
  );
}

const Chevron = ({ open, muted }) => (
  <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={muted ? "#b6c2bb" : "#5d6f64"} strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"
    style={{ flexShrink: 0, transition: "transform .25s ease", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>
    <path d="m6 9 6 6 6-6" />
  </svg>
);
...
const INTAKE_STEPS = [
  { id: "address", title: "Property address", hint: "Where is the home you're protesting?" },
  { id: "evidence", title: "County evidence packet", optional: true, hint: "Upload the county's packet for the strongest case" },
  { id: "analyze", title: "Get your recommendation", hint: "We test every angle and pick the value that holds up" },
];

function IntakeProgress({ current, narrow }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: narrow ? 8 : 12, marginBottom: 28 }}>
        ...
        return (
          <React.Fragment key={s.id}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{
                width: narrow ? 24 : 28, height: narrow ? 24 : 28, borderRadius: 999,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: narrow ? 11 : 12, fontWeight: 800,
                background: done ? "#1d6b41" : active ? "#e3efe6" : "#eef2f0",
                color: done ? "#fff" : active ? "#1d6b41" : "#b6c2bb",
                border: active ? "2px solid #1d6b41" : "2px solid transparent",
                transition: "all .2s ease",
              }}>{done ? <Check /> : i + 1}</span>
              {!narrow ? (
                <span style={{ fontSize: 13, fontWeight: active || done ? 700 : 500, color: active || done ? "#16241c" : "#9aa6a0" }}>{s.title}</span>
              ) : null}
            </div>
            {i < INTAKE_STEPS.length - 1 ? (
              <div style={{ width: narrow ? 20 : 36, height: 2, borderRadius: 999, background: done ? "#1d6b41" : "#e3ebe6", transition: "background .25s ease" }} />
            ) : null}
          </React.Fragment>
        );
      })}
...
function WizardStepShell({ n, title, optional, hint, state, summary, onEdit, narrow, children }) {
  ...
  const shellStyle = {
    borderRadius: 16,
    border: "1.5px solid " + (isActive ? "#bfe0cd" : isDone ? "#e6ebe6" : "#eef2f0"),
    background: isActive ? "#fff" : isDone ? "#f9fbf9" : "#fafbfa",
    boxShadow: isActive ? "0 8px 28px rgba(22,84,47,.08)" : "none",
    overflow: "hidden",
  ...
  return (
    <div style={shellStyle}>
      <button type="button" onClick={isDone ? onEdit : undefined} style={headerBtnStyle} aria-expanded={isActive}>
        {isDone ? (
          <span style={{
            width: 26, height: 26, borderRadius: "50%", background: "#1d6b41",
            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
          }}><Check /></span>
        ) : (
          <StepNum n={n} muted={isUpcoming} />
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 15.5, fontWeight: 700, color: isUpcoming ? "#9aa6a0" : "#16241c" }}>{title}</span>
            {optional ? (
              <span style={{ fontSize: 11, fontWeight: 700, color: "#8a988f", background: "#eef3ee", padding: "2px 8px", borderRadius: 20 }}>Optional</span>
            ) : null}
            {isDone && summary ? (
              <span style={{ fontSize: 13, fontWeight: 600, color: "#5d6f64", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: narrow ? 180 : 420 }}>{summary}</span>
            ) : null}
          </div>
          {isActive && hint ? (
            <div style={{ fontSize: 13.5, color: "#7c8a80", marginTop: 4, fontWeight: 500 }}>{hint}</div>
          ) : null}
          {isUpcoming && hint ? (
            <div style={{ fontSize: 12.5, color: "#b6c2bb", marginTop: 3, fontWeight: 500 }}>{hint}</div>
          ) : null}
        </div>
        {isDone ? (
          <span style={{ fontSize: 13, fontWeight: 700, color: "#2c8350", flexShrink: 0 }}>Edit</span>
        ) : (
          <Chevron open={isActive} muted={isUpcoming} />
        )}
      </button>
      {isActive && children ? (
        <div style={{ padding: narrow ? "0 16px 20px" : "0 20px 24px", animation: "stepReveal .35s ease" }}>
          {children}
        </div>
      ) : null}
    </div>
...
function ProgressiveIntake({
  narrow, intakeStep, setIntakeStep,
  address, setAddress, addrRef,
  files, fileRef, dragging, setDragging, onDrop, pickFiles, clearFile,
  evidenceSkipped, setEvidenceSkipped,
  error, status, isAnalyzing, analyzeStep, stepLabels,
  canAnalyze, ctaDisabled, ctaStyle, dropStyle, analyze,
}) {
  const addressReady = !!address.trim();
  const evidenceSummary = evidenceSkipped
    ? "Skipped — using TaxDrop comps"
    : files.length
      ? files.length === 1 ? files[0].name : files.length + " files added"
      : "";

  const goAddress = () => { if (!isAnalyzing) { setError(""); setIntakeStep(0); } };
  const goEvidence = () => { if (!isAnalyzing) { setError(""); setEvidenceSkipped(false); setIntakeStep(1); } };
  const advanceFromAddress = () => { if (addressReady) { setIntakeStep(1); } };
  const advanceFromEvidence = () => setIntakeStep(2);
  const skipEvidence = () => { setEvidenceSkipped(true); setFiles([]); setIntakeStep(2); };

  const stepState = (i) => (i < intakeStep ? "done" : i === intakeStep ? "active" : "upcoming");

  const secondaryBtn = {
    background: "#fff", color: "#1d6b41", border: "1.5px solid #cfe6d8",
    borderRadius: 11, padding: "12px 20px", fontSize: 14, fontWeight: 700,
    fontFamily: "inherit", cursor: "pointer",
  };
  const primaryBtn = {
    background: "linear-gradient(135deg,#27764a,#16542f)", color: "#fff", border: "none",
    borderRadius: 11, padding: "13px 22px", fontSize: 14.5, fontWeight: 700,
    fontFamily: "inherit", cursor: "pointer", boxShadow: "0 4px 12px rgba(22,84,47,.25)",
  };

  return (
    <section style={{
      background: "#fff", border: "1px solid #e6ebe6", borderRadius: 20,
      boxShadow: "0 1px 2px rgba(20,40,28,.04),0 12px 34px rgba(20,40,28,.06)",
      padding: narrow ? "22px 16px 24px" : "30px 32px 32px", marginBottom: 24,
    }}>
      <div style={{ textAlign: "center", marginBottom: 8 }}>
        <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: ".14em", color: "#2c8350" }}>
          STEP {intakeStep + 1} OF {INTAKE_STEPS.length}
        </span>
      </div>
      <IntakeProgress current={intakeStep} narrow={narrow} />

      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <WizardStepShell
          n={1} title={INTAKE_STEPS[0].title} hint={INTAKE_STEPS[0].hint}
          state={stepState(0)} summary={address.trim()} onEdit={goAddress} narrow={narrow}
        >
          <input
            ref={addrRef} value={address} onChange={(e) => setAddress(e.target.value)}
            placeholder="Street, city, state ZIP" spellCheck="false"
            style={{
              width: "100%", display: "block", border: "1.5px solid #e2e8e2", background: "#f7faf7",
              borderRadius: 12, padding: "15px 18px", marginBottom: 16, fontSize: 16, fontWeight: 600,
              color: "#16241c", fontFamily: "inherit", outline: "none",
            }}
          />
          <p style={{ margin: "0 0 18px", fontSize: 13.5, lineHeight: 1.5, color: "#7c8a80", fontWeight: 500 }}>
            Start with the address on your Notice of Appraised Value. We'll pull county records and build equity comps from there.
          </p>
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <button type="button" onClick={advanceFromAddress} disabled={!addressReady}
              style={{ ...primaryBtn, opacity: addressReady ? 1 : 0.45, cursor: addressReady ? "pointer" : "not-allowed" }}>
              Continue
            </button>
          </div>
        </WizardStepShell>
        ...
        <WizardStepShell
          n={2} title={INTAKE_STEPS[1].title} optional hint={INTAKE_STEPS[1].hint}
          state={stepState(1)} summary={evidenceSummary} onEdit={goEvidence} narrow={narrow}
        >
          <div
          ...
          >
            <input type="file" multiple
              ...
              ref={fileRef} onChange={(e) => { pickFiles(e.target.files); e.target.value = ""; setEvidenceSkipped(false); }}
              style={{ display: "none" }} />
            <div style={{ width: 50, height: 50, borderRadius: 13, background: "#e3efe6", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
              <ArrowUp s={22} />
            </div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#16241c", marginBottom: 6 }}>Drop the county's evidence — PDF, Excel, or CSV</div>
            <div style={{ fontSize: 14, color: "#7c8a80", marginBottom: 18, maxWidth: 480, marginLeft: "auto", marginRight: "auto", lineHeight: 1.5 }}>
              Drop one file or several (e.g. the cover-letter PDF plus a comparables sheet). Up to 10&nbsp;MB each.
            </div>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 8, ...primaryBtn, padding: "11px 22px", fontSize: 14 }}>
              <ArrowUp s={15} c="#fff" w={2.2} />{files.length ? "Add more files" : "Add files"}
            </span>
          </div>
            ...
            <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
              {files.map((f, idx) => (
                <div key={f.name + ":" + f.size + ":" + idx} style={{ display: "flex", alignItems: "center", gap: 14, border: "1px solid #e6ebe6", borderRadius: 12, padding: "13px 16px" }}>
                  <div style={{ width: 38, height: 38, borderRadius: 9, background: "#eef3ee", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <FileIcon c="#1d6b41" />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: 20, flexWrap: "wrap" }}>
            <button type="button" onClick={skipEvidence} style={secondaryBtn}>Skip — use TaxDrop comps</button>
            <button type="button" onClick={advanceFromEvidence} style={primaryBtn}>Continue</button>
          </div>
        </WizardStepShell>

        <WizardStepShell
          n={3} title={INTAKE_STEPS[2].title} hint={INTAKE_STEPS[2].hint}
          state={stepState(2)} narrow={narrow}
        >
          <div style={{ background: "#f7faf7", border: "1px solid #e6ebe6", borderRadius: 12, padding: "14px 16px", marginBottom: 18 }}>
            <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: ".1em", color: "#8a988f", marginBottom: 8 }}>READY TO ANALYZE</div>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: "#16241c", marginBottom: 4 }}>{address.trim()}</div>
            <div style={{ fontSize: 13.5, color: "#5d6f64", fontWeight: 500 }}>
              {evidenceSkipped
                ? "No county packet — we'll build the case from TaxDrop's equity comps."
                : files.length
                  ? files.length + " county file" + (files.length > 1 ? "s" : "") + " attached."
                  : "No files yet — we'll still run on TaxDrop comps."}
            </div>
          </div>

          {error && !isAnalyzing ? (
            <div style={{ marginBottom: 16, padding: "13px 15px", background: "#fbeceb", border: "1px solid #f0c9c5", borderRadius: 11, color: "#9e2a2a", fontSize: 13.5, fontWeight: 500 }}>{error}</div>
          ) : null}

          <button onClick={analyze} disabled={ctaDisabled} style={ctaStyle}>
            {isAnalyzing ? (
              <React.Fragment>
                <span style={{ width: 16, height: 16, border: "2.5px solid rgba(255,255,255,.4)", borderTopColor: "#fff", borderRadius: 999, display: "inline-block", animation: "spin .7s linear infinite" }} />
                Analyzing evidence…
              </React.Fragment>
            ) : (
          ...
          {isAnalyzing ? (
            <div style={{ marginTop: 22, borderTop: "1px solid #eef2f0", paddingTop: 20 }}>
              <div style={{ height: 6, borderRadius: 999, background: "#e7eeea", overflow: "hidden", marginBottom: 18 }}>
                <div style={{ width: Math.min((analyzeStep + 1) / 4, 1) * 100 + "%", height: "100%", borderRadius: 999, background: "linear-gradient(90deg,#1d6b41,#2bb0c4)", transition: "width .5s ease" }} />
              </div>
              {stepLabels.map((label, i) => {
                const done = i < analyzeStep, active = i === analyzeStep;
                return (
...
function App() {
  ...
  const reset = () => {
    setStatus("idle"); setStep(0); setIntakeStep(0); setEvidenceSkipped(false);
    setResult(null); setError(""); setFiles([]); setCadRaw(null);
  };
      ...
      <main style={{ maxWidth: 1080, margin: "0 auto", padding: narrow ? "0 16px 64px" : "0 24px 90px" }}>
        ...
        {showForm ? (
          <section style={{ textAlign: "center", padding: narrow ? "40px 0 28px" : "64px 0 40px" }}>
            ...
            <p style={{ maxWidth: 620, margin: "0 auto", fontSize: narrow ? 16 : 18, lineHeight: 1.55, color: "#5d6f64", fontWeight: 500 }}>One step at a time — address, then your county packet, then your recommendation. We test every angle and hand you the biggest reduction that holds up at hearing.</p>
          </section>
        ) : null}
        ...
        {showForm ? (
          <ProgressiveIntake
            ...
            pickFiles={pickFiles}
            clearFile={clearFile}
            evidenceSkipped={evidenceSkipped}
            setEvidenceSkipped={setEvidenceSkipped}
            error={error}
            status={status}
        ...
        {showForm && !isAnalyzing ? (
          <p style={{ textAlign: "center", margin: "0 0 64px", fontSize: 13.5, color: "#7c8a80", fontWeight: 500 }}>
            Comparing with the live intake?{" "}
            <a href="/v2" style={{ color: "#2c8350", fontWeight: 700, textDecoration: "none", borderBottom: "1px solid #bcd9c8" }}>View current v2 layout →</a>
            {" · "}
            <span onClick={loadDemo} style={{ color: "#2c8350", fontWeight: 700, cursor: "pointer", borderBottom: "1px solid #bcd9c8" }}>Preview sample recommendation →</span>
          </p>
        ) : null}
